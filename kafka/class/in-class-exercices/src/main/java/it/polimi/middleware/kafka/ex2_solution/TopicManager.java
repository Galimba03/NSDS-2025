package it.polimi.middleware.kafka.ex2_solution;
...
public class TopicManager {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (topicsNames.contains(topicName)) {
            ...
            // Wait until the topic is deleted from the list
            boolean topicStillExists = true;
            int attempts = 0;
            while (topicStillExists && attempts < 30) {
                Thread.sleep(1000);

                Set<String> currentTopics = adminClient.listTopics().names().get();
                topicStillExists = currentTopics.contains(topicName);

                attempts++;
                System.out.println("Waiting for topic deletion... " + attempts + "s");
            }

            if (topicStillExists) {
                System.out.println("Warning: Topic might not be fully deleted yet");
            }
        }

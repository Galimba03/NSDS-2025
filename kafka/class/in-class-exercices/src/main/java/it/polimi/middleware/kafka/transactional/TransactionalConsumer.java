...
public class TransactionalConsumer {
    ...
    public static void main(String[] args) {
        ...
        // It sets an additional parameter that is the "isolation level"
        // Read uncommitted => it reads things that were not committed by the consumer
        if (readUncommitted) {
            // Read all messages that are being pushed to kafka but not committed yet
            props.put(ConsumerConfig.ISOLATION_LEVEL_CONFIG, "read_uncommitted");
        } else {
            // Read all messages that are being pushed to kafka and that are commited

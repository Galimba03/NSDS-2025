...
void send_filtered_results_to_master(int my_rank, int *in, int size, int num) {
    ...
    // This portion calculate the numbers of elements in the partial array that are multiple of the element N inside. Then send this number to the original root
...
int main(int argc, char **argv)
{
    ...
    int *local_array;
    if (my_rank == 0) {
        // The num is known only to process 0 here

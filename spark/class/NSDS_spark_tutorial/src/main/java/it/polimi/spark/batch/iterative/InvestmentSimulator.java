...
public class InvestmentSimulator {
    ...
    public static void main(String[] args) {
        ...
        // Creation of the Spark Context as always
        ...
        // Convert each line in the text file into a pair of numbers (Double - Double)
        JavaRDD<Tuple2<Double, Double>> investments = textFile
                .map(w -> {
        ...
        while (sum < threshold) { // If you want to do iteration, you need to do them into the driver program
            iteration++;
            investments = investments
                    // .map() it's a transformation, so it doesn't start any job
                    .map(i -> {
    ...
    private static double sumAmount(JavaRDD<Tuple2<Double, Double>> investments) {
        return investments
                // .mapToDouble() returns a JavaDoubleRDD, so an array of doubles in other words
                .mapToDouble(a -> a._1)
                // .sum() sum all the values of a JavaDoubleRDD
                .sum();
    }

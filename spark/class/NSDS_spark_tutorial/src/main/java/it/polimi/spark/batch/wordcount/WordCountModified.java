...
public class WordCountModified {
    ...
    public static void main(String[] args) {
        ...
        final JavaRDD<String> words = lines.flatMap(s -> Arrays.asList(s.split(" ")).iterator());

        // JavaPairRDD is a giant distributed list that is contained on various workers.
        // Inside a JavaPairRDD there are objects like Tuple2
        List<Tuple2<String, Integer>> q1 = words
                .map(String::toLowerCase) // String::toLowerCase === (s -> s.toLowerCase())
...
//        Se il codice finisce con...,  Il tipo di variabile DEVE essere...,    Dove sono i dati?
//          .collect(),                 List<Tuple2<...>>,                      Nella RAM del tuo PC (Driver)
//          .take(n),                   List<Tuple2<...>>,                      Nella RAM del tuo PC (Driver)
//          .reduceByKey(...),          JavaPairRDD<...>,                       Sparsi sui Worker (Docker)
//          .mapToPair(...),            JavaPairRDD<...>,                       Sparsi sui Worker (Docker)
//          .map(...),                  JavaRDD<...>,                           Sparsi sui Worker (Docker)
        ...
        Tuple2<Integer, Integer> q3 = lines
                ...
                // ._x is the x-th element of the Tuple (i.e. a._1 is the first element of the Tuple2 a)
